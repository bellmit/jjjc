...
import com.taiji.system.service.ISysPermissionService;
import com.taiji.system.service.ISysPostService;
...
public class SysUserController extends BaseController
{
    ...
    public R<List<SysUser>> listByJob()
    {
        ...
        List<SysUser> userList = userService.selectUserListByJob();
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user)
    {
        ...
        if(StringUtils.isNotEmpty(user.getPassword())){
            ...
            user.setCreateBy("job");
        }

        return toAjax(userService.insertUser(user));
    }

    @Log(title = "用户同步", businessType = BusinessType.INSERT)
    @PostMapping("/jobAdd")
    public AjaxResult feginAdd(@Validated @RequestBody SysUser user)
    {
        if (UserConstants.NOT_UNIQUE.equals(userService.checkUserNameUnique(user.getUserName())))
        {
            //如果已存在，则修改
            return toAjax(userService.updateUserByOaId(user));
        }
            user.setCreateBy("job");
        return toAjax(userService.insertUser(user));
    }
